...
public:
	BattleRoom(int roomCount, Player& player, Deck* playerDeck); // 원래는 BattleRoom(Player& player, std::vector<std::unique_ptr<Enemy>> enemies); Deck 생성자 추가
	int Run();
	void Reward();
	void PlayerTurnRun();
    void WaitForEnter();
    std::string RarityToString(CardRarity rarity) const;
    std::vector<CardData> PackageCards(Deck* playerDeck);
